...
const TextToSpeechComponent = forwardRef(
  ({ text, lang, autoSpeak, callbackFinished }: Props, ref) => {
    ...
    if (isSpeaking) {
      return (
        <Button variant="ghost" size="icon" onClick={(e) => stopSpeak()}>
          <SpeakerOffIcon className="w-5 h-5" />
        </Button>
      );
    } else {
      return (
        <Button variant="ghost" size="icon" onClick={(e) => speak()}>
